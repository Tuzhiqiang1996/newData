...
function draw_palet() {
  var palet_container = d3
    .select("svg")
    .append("g")
    .attr("id", "palet_container");
...
function show_choosed_bar() {
  ...
  if (palet.attr("data-palet") == "on") {
    ...
    for (var i = 0; i < 5; i++) {
      d3.selectAll("#graphe_container" + last_g + ">.color_rect" + i).attr(
        "fill",
        deg_color(last_color)[i]
      );
    }
  } else {
    d3.selectAll("#graphe_container" + last_g).attr(
      "data-color",
      deg_color(p_color)[2]
    );
    for (var i = 0; i < 5; i++) {
